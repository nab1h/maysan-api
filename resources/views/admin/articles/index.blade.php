<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-900 leading-tight">إدارة المقالات</h2>
            <a href="{{ route('admin.articles.create') }}"
                ...
                <i class="fa-solid fa-plus text-xs"></i> كتابة مقال جديد
            </a>
        </div>
    </x-slot>
                    ...
                    <table class="w-full text-right border-collapse">
                        <thead>
                            <tr class="bg-gray-50 text-gray-500 text-sm border-b border-gray-200">
                                ...
                                <th class="p-4 font-medium">المشاهدات</th>
                        ...
                        <tbody class="text-sm divide-y divide-gray-100">
                            ...
                            <tr class="hover:bg-gray-50 transition duration-200 group">
                                ...
                                <td class="p-4">
                                    @if($item->image)
                                    <img src="{{ Storage::url($item->image) }}" class="w-16 h-10 rounded-lg object-cover border border-gray-200">
                                    @else
                                ...
                                <td class="p-4">
                                    ...
                                    <div class="text-xs text-gray-400 font-mono mt-1" dir="ltr">{{ $item->slug }}</div>
                                </td>
                                <td class="p-4">
                                ...
                                <td class="p-4 text-gray-500">
                                    <div class="flex items-center gap-1">
                                        <i class="fa-solid fa-eye text-xs"></i> {{ $item->views }}
                                    </div>
                                </td>
                                <td class="p-4 text-gray-500 text-xs">{{ $item->created_at->format('Y/m/d') }}</td>
                                <td class="p-4">
                                    <div class="flex items-center justify-center gap-2 opacity-100 lg:opacity-0 group-hover:opacity-100 transition">
                                        <a href="{{ route('admin.articles.edit', $item->id) }}" class="p-2 rounded-lg bg-gray-100 text-blue-500 hover:text-white hover:bg-blue-600 transition" title="تعديل">
                                            <i class="fa-solid fa-pen-to-square"></i>
                                        </a>
                                        <form action="{{ route('admin.articles.destroy', $item->id) }}" method="POST" onsubmit="return confirm('هل أنت متأكد من حذف هذا المقال؟');">
                                            @csrf @method('DELETE')
                            ...
                            @empty
                            <tr>
                                <td colspan="7" class="p-12 text-center border-0">
                                    <div class="flex flex-col items-center gap-3">
                                        <div class="w-16 h-16 rounded-full bg-gray-100 flex items-center justify-center">
                                            <i class="fa-solid fa-newspaper text-2xl text-gray-300"></i>
                                        </div>
                                        <p class="text-gray-500 font-medium">لا توجد مقالات بعد</p>
                                        <a href="{{ route('admin.articles.create') }}" class="text-[#135158] font-semibold text-sm hover:underline">اكتبي أول مقال الآن</a>
                                    </div>
                                </td>
                            </tr>
                            @endforelse
