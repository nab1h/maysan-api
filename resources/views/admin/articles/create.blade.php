<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <h2 class="font-semibold text-xl text-gray-900 leading-tight">كتابة مقال جديد</h2>
        </div>
    </x-slot>
                ...
                <form action="{{ route('admin.articles.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
                        ...
                        <div>
                            ...
                            <input type="text" name="title" id="title" value="{{ old('title') }}" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#135158]/20 focus:border-[#135158] transition" placeholder="أدخلي عنوان المقال">
                        </div>
                    </div>
                    ...
                    <div class="mb-5">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <input type="text" name="slug" id="slug" value="{{ old('slug') }}" dir="ltr" class="flex-1 border border-gray-300 rounded-lg px-4 py-3 text-left focus:outline-none focus:ring-2 focus:ring-[#135158]/20 focus:border-[#135158] transition font-mono text-sm" placeholder="how-to-care-for-skin">
                        </div>
                    </div>

                    <!-- صورة الغلاف وملف الورد -->
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
                        <div>
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">
                                <i class="fa-solid fa-file-word text-blue-500 ml-1"></i> رفع ملف وورد (اختياري)
                            </label>
                            ...
                            <p class="mt-1 text-xs text-gray-400">لو رفعتي ملف وورد، هيستبدل المحتوى اللي تحت</p>
                        </div>
                    </div>
                    ...
                    <div class="mb-5">
                        ...
                        <textarea name="excerpt" id="excerpt" rows="2" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#135158]/20 focus:border-[#135158] transition resize-none" placeholder="فكرة سريعة عن المقال تظهر في كارت المقال...">{{ old('excerpt') }}</textarea>
                    </div>

                    <!-- محتوى المقال (مع TinyMCE) -->
                    <div class="mb-6">
                        <label for="content" class="block text-sm font-semibold text-gray-700 mb-2">محتوى المقال</label>
                        <textarea name="content" id="content" rows="15" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#135158]/20 focus:border-[#135158] transition">{{ old('content') }}</textarea>
                    </div>
                    ...
                    <div class="flex items-center gap-3 pt-4 border-t border-gray-100">
                        <button type="submit" class="inline-flex items-center gap-2 bg-[#135158] text-white px-6 py-3 rounded-lg hover:bg-[#0a3a40] transition font-semibold">
                            <i class="fa-solid fa-check text-sm"></i> نشر المقال
                        </button>
    ...
    const slug = title
    .replace(/[^\u0621-\u064Aa-zA-Z0-9\s-]/g, '') // إزالة الحروف الخاصة
    .replace(/\s+/g, '-') // تحويل المسافات لـ -
    .toLowerCase();
