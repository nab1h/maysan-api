<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <h2 class="font-semibold text-xl text-gray-900 leading-tight">تعديل الفرع</h2>
        </div>
    </x-slot>
            ...
            <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-6 sm:p-8">
                <form action="{{ route('admin.branches.update', $branch->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div>
                            ...
                            <select name="location_id" id="location_id" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#135158]/20 focus:border-[#135158] transition bg-white">
                                ...
                                <option value="{{ $loc->id }}" {{ old('location_id', $branch->location_id) == $loc->id ? 'selected' : '' }}>{{ $loc->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $branch->name) }}" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#135158]/20 focus:border-[#135158] transition">
                        </div>

                        <div>
                            <label for="phone" class="block text-sm font-semibold text-gray-700 mb-2">رقم الهاتف <span class="text-red-500">*</span></label>
                            <input type="text" name="phone" id="phone" value="{{ old('phone', $branch->phone) }}" dir="ltr" class="w-full border border-gray-300 rounded-lg px-4 py-3 text-left focus:outline-none focus:ring-2 focus:ring-[#135158]/20 focus:border-[#135158] transition">
                        </div>
                        ...
                        <div>
                            ...
                            <div class="flex items-center gap-3">
                                @if($branch->image)
                                <img src="{{ Storage::url($branch->image) }}" class="w-12 h-12 rounded-lg object-cover border border-gray-200">
                                @endif
                                <input type="file" name="image" accept="image/*" class="flex-1 text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-[#135158]/10 file:text-[#135158] hover:file:bg-[#135158]/20 transition cursor-pointer">
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 mt-1 p-4 bg-gray-50 rounded-lg border border-gray-200">
                            ...
                            <label class="flex items-center gap-2 p-2 rounded-md hover:bg-gray-100 cursor-pointer transition">
                                <input type="checkbox" name="departments[]" value="{{ $department->id }}"
                                    ...
                                    {{ in_array($department->id, old('departments', $branchDepartments)) ? 'checked' : '' }}>
                    ...
                    <div class="mt-5">
                        ...
                        <textarea name="address" id="address" rows="2" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#135158]/20 focus:border-[#135158] transition resize-none">{{ old('address', $branch->address) }}</textarea>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 mt-5">
                        <div>
                            ...
                            <input type="url" name="instagram_url" value="{{ old('instagram_url', $branch->instagram_url) }}" dir="ltr" class="w-full border border-gray-300 rounded-lg px-4 py-3 text-left text-sm focus:outline-none focus:ring-2 focus:ring-[#135158]/20 focus:border-[#135158] transition">
                        </div>
                        <div>
                            ...
                            <input type="text" name="google_map_url" value="{{ old('google_map_url', $branch->google_map_url) }}" dir="ltr" class="w-full border border-gray-300 rounded-lg px-4 py-3 text-left text-sm focus:outline-none focus:ring-2 focus:ring-[#135158]/20 focus:border-[#135158] transition">
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-3 pt-6 mt-6 border-t border-gray-100">
                        <button type="submit" class="inline-flex items-center gap-2 bg-[#135158] text-white px-6 py-3 rounded-lg hover:bg-[#0a3a40] transition font-semibold">
                            <i class="fa-solid fa-check text-sm"></i> تحديث الفرع
                        </button>
